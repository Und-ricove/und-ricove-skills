...
## Ставить обновления или только докладывать — выбор пользователя

По умолчанию скилл **только докладывает**: хук и скрипт `update-check.ps1`
сами ничего не устанавливают, обновление — решение пользователя.

Если пользователь явно разрешил это в своём `CLAUDE.md` (например:
«обновления Claude Code ставить сразу, без вопроса»), Claude действует так:

- **CLI терминала** — первым действием сессии, как только хук показал
  «⬆ ОБНОВЛЕНИЯ», запускает `claude update` (npm, около 30 секунд; хук
  этого не делает — его таймаут 25 секунд) и докладывает одной строкой:
  «CLI 2.1.273 → 2.1.276».
- **Desktop** — пакет уже скачан апдейтером и ставится только полным
  выходом из приложения (правый клик по иконке Claude в трее → выход, затем
  запуск). Выход обрывает текущую сессию, поэтому Claude его не делает:
  напоминает одной строкой «Desktop 2.x скачана, выйдите через трей и
  запустите снова». Руку прикладывает пользователь.
- **CLI внутри Desktop** обновляется вместе с Desktop, отдельно не трогать.

Предупреждения:

- Без явного разрешения в `CLAUDE.md` — только доклад, даже для «мелкого»
  патча. Хук и скрипт пака ничего не ставят ни в каком режиме: `claude update`
  запускает сам Claude в сессии, и только по этому разрешению.
- Если у вас открыто несколько сессий Claude Code, `claude update` запускает
  одна — та, что первой увидела «⬆»; вторая сверяет `claude --version` и
  молчит (два npm одновременно — гонка; см. скилл `multisession` из плагина
  `methods`, если он у вас установлен).
...
- 18.09.2026 (вечер) — политика: по умолчанию только доклад; автообновление CLI терминала — только по явному разрешению пользователя в его `CLAUDE.md`; Desktop — напоминание о полном выходе через трей.
