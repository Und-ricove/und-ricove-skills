...
- **Закрытие окна ≠ перезапуск (18.09.2026):** после «перезапуска» окном
  установленная осталась 2.2553.0 (`CLAUDE_CODE_DESKTOP_APP_VERSION`), а
  журнал снова написал «Update downloaded and ready to install 2.2553.1»
  (16:27) и «Staged … still current» (16:37). Ставится только ПОЛНЫМ
  выходом: правый клик по иконке Claude в трее → выход, затем запуск.
  Напоминать пользователю именно так, не «перезапусти».
- **`claude --version` в Desktop-сессии показывает ТЕРМИНАЛЬНЫЙ CLI:**
  `which claude` → `~/.local/bin/claude` (PATH), хотя сессию выполняет CLI
  внутри Desktop (`CLAUDE_CODE_ENTRYPOINT=claude-desktop`). Версию CLI
  внутри Desktop смотреть по папкам `%APPDATA%\Claude\claude-code\`
  (18.09: 2.1.271 и 2.1.274 при терминальном 2.1.276) — скрипт так и делает.
...
- 18.09.2026 — две ловушки выше (закрытие окна ≠ выход; `claude --version` в Desktop-сессии — терминальный CLI). Проверка того дня: CLI терминала 2.1.276 актуален, CLI внутри Desktop 2.1.274, Desktop 2.2553.0 при скачанной 2.2553.1 — ждёт полного выхода через трей.
